...
function IssueItem({issue}) {
    ...
    const toggleShowDetail = useCallback(() => {
        setShowDetail(detail => !detail) // 使用callback更新state，逃避闭包
    }, []);
    return (
        <div>
            <div className="issue">
...
function makeQuery(user, state, labels) {
    let creatorStr = user ? `creator=${user}` : '';
    ...
    return (
        <div className="root">
            <div className="search">
                ...
                <Select style={{width: 200, marginLeft: 20}} placeholder="状态" onChange={handleStateChange}
                        value={state}>
                    <Option value="all">all</Option>
                    <Option value="open">open</Option>
                    <Option value="closed">closed</Option>
                </Select>
                <Select mode="multiple" style={{flexGrow: 1, marginLeft: 20, marginRight: 20}} placeholder="Label"
                        onChange={handleLabelChange} value={label}>
                    {
